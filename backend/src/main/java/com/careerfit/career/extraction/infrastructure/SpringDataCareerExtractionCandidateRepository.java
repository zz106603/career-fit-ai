...
interface SpringDataCareerExtractionCandidateRepository extends JpaRepository<CareerExtractionCandidateEntity, UUID> {
    ...
    List<CareerExtractionCandidateEntity> findAllByUserIdAndAnalysisIdAndStatusNotOrderByCreatedAtAsc(
            UUID userId, UUID analysisId, CareerExtractionCandidateStatus status);
